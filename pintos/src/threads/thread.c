...
thread_unblock (struct thread *t) 
{
  ...
  list_push_back (&ready_list, &t->elem);
	//if thread_current()->priority is smaller than ready_list's highest priority
	//thread_yield() to ready list's highest priority
	//need a function to find the thread in ready_list's with the highest priority
...
//Implements list_less_func from list.h, necessary to call list_insert_ordered
bool compareList(const struct list_elem *a, const struct list_elem *b, void *aux) {
	struct thread *A = list_entry(a, struct thread, elem);
	struct thread *B = list_entry(b, struct thread, elem);
	if(A->timerTick < B ->timerTick) {
		return true;
	}
	else {
		return false;
